...
/**
 * External functions for managing the embedded editor in mod_exelearning.
 *
...
class manage_embedded_editor extends external_api {
     ...
     *
     * Requires both moodle/site:config AND mod/exelearning:manageembeddededitor
    ...
    public static function execute_action(string $action): array {
        ...
        if (!in_array($action, $validactions, true)) {
            throw new \invalid_parameter_exception(
                get_string('invalidaction', 'mod_exelearning', $action)
            );
        }
    ...
    public static function get_action_success_string(string $action): string {
        ...
        return get_string($map[$action], 'mod_exelearning');
    }
    ...
    public static function get_status(bool $checklatest): array {
        ...
        // Install lock state.
        $locktime = get_config('exelearning', embedded_editor_installer::CONFIG_INSTALLING);
        $installing = false;
        $installstale = false;
