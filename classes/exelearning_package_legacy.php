...
class exelearning_package_legacy {
    ...
    public static function validate_package(\stored_file $file) {
        ...
        if (!self::is_valid_package_file($file)) {
            $errors['packagefile'] = get_string('badexelearningpackage', 'mod_exelearning');
            return $errors;
        }
    ...
    public static function expand_package($package) {
        ...
        $fs = get_file_storage();
        $fs->delete_area_files($package->get_contextid(), 'mod_exelearning', 'content');
    ...
    public static function get_mainfile(array $contentlist, int $contextid, int $itemid = 0) {
        ...
        foreach ($mainfilenames as $item) {
            $mainfile = $fs->get_file($contextid, 'mod_exelearning', 'content', $itemid, $filepath, $item);
            if ($mainfile !== false) {
                return $mainfile;
            }
        }
