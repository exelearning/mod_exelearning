...
/**
 * Embedded editor installer for mod_exelearning.
 *
...
class embedded_editor_installer {
    ...
    private function extract_latest_version_from_feed(string $feedbody): string {
        if ($feedbody === '') {
            throw new \moodle_exception('editorgithubparseerror', 'mod_exelearning');
        }
        ...
        if ($version === null) {
            throw new \moodle_exception('editorgithubparseerror', 'mod_exelearning', '', $candidate);
        }
    ...
    public function download_to_temp(string $url): string {
        ...
        if ($result === false || $curl->get_errno()) {
            ...
            throw new \moodle_exception('editordownloaderror', 'mod_exelearning', '', $curl->error);
        }
    ...
    public function validate_zip(string $filepath): void {
        ...
        if ($handle === false) {
            throw new \moodle_exception('editorinvalidzip', 'mod_exelearning');
        }
    ...
    public function extract_to_temp(string $zippath): string {
        if (!class_exists('ZipArchive')) {
            throw new \moodle_exception('editorzipextensionmissing', 'mod_exelearning');
        }

        $tmpdir = make_temp_directory('mod_exelearning/extract-' . random_string(12));
    ...
    public function normalize_extraction(string $tmpdir): string {
        ...
        throw new \moodle_exception('editorinvalidlayout', 'mod_exelearning');
    }
    ...
    public function store_metadata(string $version): void {
        set_config(self::CONFIG_VERSION, $version, 'exelearning');
        set_config(self::CONFIG_INSTALLED_AT, date('Y-m-d H:i:s'), 'exelearning');
    }
    ...
    public function clear_metadata(): void {
        unset_config(self::CONFIG_VERSION, 'exelearning');
        unset_config(self::CONFIG_INSTALLED_AT, 'exelearning');
    }
