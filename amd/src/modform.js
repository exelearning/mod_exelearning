...
/**
 * Javascript helper function for mod_exelearning module form.
 *
 * @module      mod_exelearning/modform
...
const initialize = () => {
    Prefetch.prefetchStrings('mod_exelearning', ['exeweb:editonlineanddisplay', 'exeweb:editonlineandreturntocourse']);
    ...
    document.querySelector(SELECTORS.ORIGIN).addEventListener('change', (e) => {
        ...
        if (e.target.value == 'exeonline') {
            getString('exeweb:editonlineanddisplay', 'mod_exelearning')
                .then((label) => {
                    buttonDisplay.value = label;
                    return;
                })
                .catch();
            getString('exeweb:editonlineandreturntocourse', 'mod_exelearning')
                .then((label) => {
